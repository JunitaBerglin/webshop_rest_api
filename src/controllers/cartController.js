...
exports.getAllCarts = async (req, res) => {
  ...
  if (!carts) throw new NotFoundError("There's no carts to show");
...
exports.createNewCart = async (req, res) => {
  ...
  if (!cartName || cartName.toString().length === 0)
    throw new BadRequestError("You must write a name.");
...
exports.updateCartById = async (req, res) => {
  ...
  const { cartName, totalPrice, products } = req.body;
...
exports.deleteOneProductFromCart = async (req, res) => {
  ...
  const cart = await Cart.findById(cartId);
  if (!cart) throw new NotFoundError("This cart doesn't exist");
  ...
  if (productIndex < 0)
    throw new BadRequestError("Already empty! :-)");
